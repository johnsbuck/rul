...
elseif opt.loss == 'mse' then
   ...
   if trainData then
      ...
      local trsize = (#trainData.target)[1]
      local trlabels = torch.Tensor( trsize, noutputs )
      trlabels:fill(-1)
      for i = 1,trsize do
         trlabels[{ i,trainData.target[i] }] = 1
      end
      trainData.target = trlabels
  
      -- convert test labels
      local tesize = (#testData.target)[1]
      local telabels = torch.Tensor( tesize, noutputs )
      telabels:fill(-1)
      for i = 1,tesize do
         telabels[{ i,testData.target[i] }] = 1
      end
      testData.target = telabels
   end
   --]]
elseif opt.loss == 'rmse' then
  ---
  -- @todo Create rmse criterion
  error('unimplemented -loss')
  
elseif opt.loss == 'phm' then
  ---
  -- @todo Create score function criterion
  error('unimplemented -loss')
